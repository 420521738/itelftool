...
def collect():
    ...
    for key in filter_keys:
        try:
            # subprocess.check_output执行之后的结果为bytes，如果不进行解码，会使下面的split方法失败，所以需要添加decode解码
            cmd_res = subprocess.check_output("sudo dmidecode -t system|grep '%s'" %key,shell=True).decode()
...
def raminfo():
    ...
    raw_list = raw_data.decode().split("\n")
    raw_ram_list = []
    item_list = []
    ...
    # subprocess.check_output执行之后的结果为bytes，如果不进行解码，会使下面的split方法失败，所以需要添加decode解码
    raw_total_size = raw_total_size.decode().split(":")
...
def osinfo():
    distributor = subprocess.check_output("lsb_release -a|grep 'Distributor ID'",shell=True)
    # subprocess.check_output执行之后的结果为bytes，如果不进行解码，会使下面的split方法失败，所以需要添加decode解码
    distributor = distributor.decode().split(":")
    #print('121212121',distributor)
    #distributor = commands.getoutput(" lsb_release -a|grep 'Distributor ID'").split(":")
    release  = subprocess.check_output("lsb_release -a|grep Description",shell=True)
    # subprocess.check_output执行之后的结果为bytes，如果不进行解码，会使下面的split方法失败，所以需要添加decode解码
    release = release.decode().split(":")
...
def cpuinfo():
    ...
    #print('0000000000000',raw_data,type(raw_data))
    ...
    #print('22222222222',data,type(data))
...
class DiskPlugin(object):
    ...
    def linux(self):
        data = []
        item_data = {}
        shell_command = "fdisk -l | grep 'Disk /' | awk '{print $3}'"
        output = subprocess.check_output(shell_command,shell=True).decode()
        item_data['iface_type']  = 'unknown'
        item_data['slot']  = 'unknown'
        item_data['sn']  = 'unknown'
        item_data['model']  = 'unknown'
        item_data['manufactory']  = 'unknown'
        item_data['capacity']  = round(output,2)
        ...
        try:
            ...
            print('33333333333333333test',script_path)
            shell_command = "sudo %s/MegaCli  -PDList -aALL" % script_path
            output = subprocess.check_output(shell_command,shell=True).decode()
            print('4444444444444444test',output)
